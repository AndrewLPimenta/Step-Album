...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-xl text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        // btn-brand: gradiente azul->ambar (globals.css), o mesmo da pilula
        // ativa da nav — e' o unico elemento solido da tela.
        default: "btn-brand active:scale-[0.98]",
        destructive:
          "bg-destructive text-destructive-foreground shadow-sm hover:bg-destructive/90",
        outline:
          "glass-field hover:border-[hsl(var(--brand-blue)/0.45)] hover:text-foreground",
        secondary:
          "glass-chip text-foreground hover:border-[hsl(var(--brand-blue)/0.35)]",
        ghost:
          "hover:bg-[hsl(var(--brand-blue)/0.08)] hover:text-foreground",
