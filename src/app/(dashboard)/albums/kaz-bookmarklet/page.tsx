...
const BOOKMARKLET = `javascript:(function(){var $=window.jQuery;if(!$){alert('Erro: jQuery não encontrado.');return;}var T=$('#DataTableDiagramacoes');if(!T.length){alert('Vá até a página de Diagramações do Kaz.');return;}var dt=T.DataTable();function getKazId(tr,dtApi){var trId=$(tr).attr('id');if(trId){var tm=/^(?:row_)?(\\d+)$/.exec(trId);if(tm)return tm[1];}try{var rd=dtApi.row(tr).data();if(rd){var k=rd.DT_RowId||rd.id||rd.formando_id||rd.usuario_id||null;if(k)return String(k).replace(/^row_/,'');}}catch(e){}var h=$(tr).find('a[href*="/detalhe_formando/"]').first().attr('href')||'';var m=h.match(/\\/([0-9]+)\\/?$/);if(m)return m[1];return null;}function ex(){var dtApi=T.DataTable();var rows=[];T.find('tbody tr').each(function(){var tds=$(this).find('td');if(tds.length<4)return;var c=(tds.eq(1).find('[data-bs-original-title]').attr('data-bs-original-title')||tds.eq(1).text()).trim();if(!/^\\d{9}$/.test(c))return;rows.push({class_code:c.slice(0,5),student_code:c.slice(5),student_name:(tds.eq(2).find('[data-bs-original-title]').attr('data-bs-original-title')||tds.eq(2).text()).trim(),faculty:(tds.eq(3).find('[data-bs-original-title]').attr('data-bs-original-title')||tds.eq(3).text()).trim(),kaz_id:getKazId(this,dtApi)});});if(!rows.length){alert('Nenhum álbum encontrado.');return;}var withId=rows.filter(function(r){return r.kaz_id;}).length;navigator.clipboard.writeText(JSON.stringify(rows)).then(function(){var d=document.createElement('div');d.style.cssText='position:fixed;top:20px;right:20px;background:#16a34a;color:#fff;padding:14px 20px;border-radius:10px;font-size:15px;z-index:999999;box-shadow:0 4px 16px rgba(0,0,0,.25);font-family:sans-serif';d.innerHTML='✅ '+rows.length+' álbuns copiados! ('+withId+' com ID Kaz)<br><small>Cole no StepAlbum → Importar da Kaz<\/small>';document.body.appendChild(d);setTimeout(function(){d.remove();},4000);}).catch(function(){prompt('Copie o JSON:',JSON.stringify(rows));});}var i=dt.page.info();if(i.recordsTotal>i.length&&i.length>0){dt.one('draw',function(){setTimeout(ex,200);});dt.page.len(500).draw();}else{ex();}})();`;
