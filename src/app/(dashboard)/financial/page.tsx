...
import { listSentAlbums, buildCycleSummaries } from "@/lib/queries";
import {
  Card,
  CardContent,
  CardHeader,
...
export default async function FinancialPage() {
  ...
  const [sentAlbums, usersRes] = await Promise.all([
    listSentAlbums(),
  ...
  // Determine current open cycle and most recently closed cycle
  ...
  const openSummary = summaries.find(
    (s) => s.paymentDate === currentPaymentDate,
  );
  ...
  const historySummaries = summaries.filter(
    (s) =>
      s.isPast &&
      s.paymentDate !== currentPaymentDate &&
      s.paymentDate !== prevPaymentDate,
  );

  const daysUntilClose = daysUntil(toDateOnly(currentCycle.cycleEnd), today);
  const daysUntilOpen = openSummary
    ? daysUntil(currentPaymentDate, today)
    : null;
  const daysUntilClosed = closedSummary
    ? daysUntil(prevPaymentDate, today)
    : null;
    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Projeção de pagamentos por ciclo quinzenal
        </p>
      </div>

      {/* ── Cycle cards ── */}
      <div className="grid gap-4 sm:grid-cols-2">
        {/* Closed cycle — fixed amount */}
        <Card
          className={`border-2 ${closedSummary ? "border-amber-500/40 bg-amber-500/5" : "border-border/40 bg-card/30 opacity-60"}`}
        >
            ...
            {closedSummary ? (
              <>
                <p className="text-3xl font-bold tabular-nums">
                  {formatBRL(closedSummary.total)}
                </p>
                <p className="text-xs text-muted-foreground">
                  {closedSummary.count} álbum
                  {closedSummary.count !== 1 ? "ns" : ""} enviado
                  {closedSummary.count !== 1 ? "s" : ""}
                </p>
                {closedSummary.byUser.length > 0 && (
        ...
        <Card className="border-2 border-[hsl(var(--brand-blue)/0.4)] bg-[hsl(var(--brand-blue)/0.05)]">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant="outline"
                className="text-xs border-[hsl(var(--brand-blue)/0.4)] text-[hsl(var(--brand-blue))]"
              >
          ...
          <CardContent className="space-y-3">
            <p className="text-3xl font-bold tabular-nums">
              {formatBRL(openSummary?.total ?? 0)}
            </p>
            <p className="text-xs text-muted-foreground">
              {openSummary?.count ?? 0} álbum
              {(openSummary?.count ?? 0) !== 1 ? "ns" : ""} enviado
              {(openSummary?.count ?? 0) !== 1 ? "s" : ""}
              {" · "}cresce conforme envios
            </p>
            {openSummary && openSummary.byUser.length > 0 && (
              <div className="space-y-1 pt-1 border-t border-border/40">
      ...
      {/* ── Next upcoming payments preview ── */}
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {openSummary && daysUntilOpen !== null && (
            <div className="flex items-center gap-2 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2">
              ...
              <span className="text-xs">
                <span className="font-semibold">
                  {formatBRL(openSummary.total)}
                </span>{" "}
                projetado para{" "}
                <span className="font-semibold">
                  {formatDate(currentPaymentDate)}
                </span>
              </span>
            </div>
          )}
      ...
      {summaries.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/60 py-16 text-center">
          ...
          <p className="text-sm font-medium">Nenhum álbum enviado ainda</p>
          <p className="text-xs text-muted-foreground mt-1">
            O valor aparece aqui conforme os álbuns forem marcados como
            enviados.
          </p>
        </div>
      )}
